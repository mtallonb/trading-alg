...
SERVER_ADDRESS = "wss://ws.xtb.com/real"
SERVER_ADDRESS_STREAM = "wss://ws.xtb.com/realStream"
...
class XTB:
        ...
        for rows in wb.active.iter_rows(min_row=0, max_row=1000000):
            ...
            for cell in rows:
                if i == 0 and cell.value is None:
                    return temp1
                elif i == 0 and not cell.value:
                    temp["datetime"] = cell.value
                elif i == 1 and not cell.value:
                    temp["open"] = cell.value
                elif i == 2 and not cell.value:
                    temp["close"] = cell.value
                elif i == 3 and not cell.value:
                    temp["high"] = cell.value
                elif i == 4 and not cell.value:
                    temp["low"] = cell.value
                i += 1
            temp1.append(temp)
    ...
    def connect(self):
        try:
            self.ws = websocket.create_connection(SERVER_ADDRESS)
            # Success
            return True
        except:
            # Error
